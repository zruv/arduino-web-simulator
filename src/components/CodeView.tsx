...
export const CodeView: React.FC<CodeViewProps> = ({ canvasComponents }) => {
  const generateArduinoCode = useMemo(() => {
    ...
    leds.forEach((led, index) => {
      ...
      loopCode += `  // Control LED ${index + 1} here\n`;
    });
    ...
    buttons.forEach((button, index) => {
      ...
      if (leds[index]) { // Simple example: button controls corresponding LED
        loopCode += `    digitalWrite(ledPin${index + 1}, HIGH);\n`;
      } else {
        loopCode += `    // Button ${index + 1} is pressed\n`;
      }
      loopCode += `  } else {\n`;
      if (leds[index]) {
        loopCode += `    digitalWrite(ledPin${index + 1}, LOW);\n`;
      } else {
        loopCode += `    // Button ${index + 1} is released\n`;
      }
      loopCode += `  }\n`;
    });
  ...
  Generated by FOSSEE Arduino Simulator
*/
...
${loopCode}
  ...
  // Split code into lines for display with line numbers
                    ...
                    <div key={i} className="text-xs font-mono leading-6 h-6 whitespace-pre">
                        ...
                        {line.split(' ').map((word, j) => {
                            ...
                            if (word.startsWith('//') || word.startsWith('/*') || word.startsWith('*')) return <span key={j} className="text-slate-500">{word} </span>;
                             // Handle comments that span rest of line (simple check)
